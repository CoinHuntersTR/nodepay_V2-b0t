...
def load_config():
  try:
      ...
      raise SystemExit("Exiting due to failure in loading config")
...
if __name__ == '__main__':
  ...
  if "TMUX" not in os.environ:
      ...
      config = load_config()
      np_token = config.get("np_token")
      proxy_file = config.get("proxy_file")

      if not np_token or not proxy_file:
          logger.error("NP_TOKEN or proxy file path is missing in the config.")
