...
def ensure_screen_installed():
  result = subprocess.run(['which', 'screen'], stdout=subprocess.PIPE)
  if result.returncode != 0:
      logger.info("screen is not installed. Installing screen...")
      subprocess.run(['apt-get', 'update'], check=True)
      subprocess.run(['apt-get', 'install', '-y', 'screen'], check=True)
  else:
      logger.info("screen is already installed.")

def run_in_screen(session_name, script_path):
  subprocess.run(['screen', '-dmS', session_name, 'bash', '-c', f'python3 {script_path}'])
  logger.info(f"Started screen session '{session_name}' running the script.")
...
if __name__ == '__main__':
  ...
  if "STY" not in os.environ:
      ...
      run_in_screen('Nodepay', script_path)
  else:
